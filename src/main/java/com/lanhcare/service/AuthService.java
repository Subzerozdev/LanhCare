...
public class AuthService {
    ...
    @Transactional
    public String loginGoogleForMobile(String email, String name) {
        Account googleAccount = accountRepository.findByEmail(email).orElse(null);

        // Create account if not exist one
        if (googleAccount == null) {
            googleAccount = accountRepository.save(Account.builder()
                    .email(email)
                    .fullname(name)
                    .role(AccountRole.USER)
                    .status(AccountStatus.ACTIVE)
        ...
        if (googleAccount.getStatus().equals(AccountStatus.SUSPENDED)) {
